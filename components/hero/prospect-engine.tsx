...
export function ProspectEngine({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      {/*
        Pipeline board. Below lg this is a horizontal strip, not a wrapped
        grid: five stages folded into two columns stops reading as a pipeline
        and becomes an arbitrary grid of tags. A row clipped at the edge is
        how the real board behaves on a phone, and it keeps the left-to-right
        order that carries the whole meaning.
      */}
      <div className="relative">
        ...
        />
        <div className="flex gap-px bg-border lg:grid lg:grid-cols-5">
          {stages.map((stage) => (
            <div
              key={stage.name}
              className="w-[62%] shrink-0 bg-card p-4 sm:w-[38%] sm:p-5 lg:w-auto"
            >
